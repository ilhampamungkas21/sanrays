    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link
            href={`/admin/kelola/${event_id}/hasil-pertanyaan`}
      ...
      <div className="flex gap-1 p-1 bg-gray-100 rounded-xl w-fit">
        {[
          ...
          { key: "pertanyaan", label: "Hasil Pertanyaan" },
        ].map((tab) => (
          <button
            key={tab.key}
      ...
      {/* Hasil Pertanyaan Tab - Redirect to dedicated page */}
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-lg font-bold text-gray-900">Hasil Pertanyaan Kustom</h2>
                  <p className="text-sm text-gray-500 mt-1">Lihat statistik dan jawaban lengkap</p>
                </div>
                <Link
                  href={`/admin/kelola/${event_id}/hasil-pertanyaan`}
                  className="flex items-center gap-2 px-4 py-2 bg-purple-500 text-white rounded-lg text-sm font-medium hover:bg-purple-600 transition-colors"
          ...
          {/* Quick Preview of Custom Questions */}
          {customQuestions.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {customQuestions.slice(0, 6).map((q: any) => {
                const answers = feedback
                  .map(f => f.customAnswers?.find((a: any) => a.questionId === q.id))
                  .filter(Boolean);
                return (
                  <div key={q.id} className="bg-white rounded-xl border border-gray-200 p-4">
                    <div className="flex items-start gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                        q.questionType === 'rating' ? 'bg-yellow-100 text-yellow-600' :
                        q.questionType === 'multiple_choice' ? 'bg-purple-100 text-purple-600' :
                        'bg-blue-100 text-blue-600'
                      }`}>
                        {q.questionType === 'rating' ? '⭐' :
                         q.questionType === 'multiple_choice' ? '🔘' : '✏️'}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-900 line-clamp-2">{q.questionText}</p>
                        <p className="text-xs text-gray-500 mt-1">{answers.length} jawaban</p>
                      </div>
                    </div>
                  </div>
                );
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
              <p className="text-gray-500">Belum ada pertanyaan kustom</p>
            </div>
          )}
        </div>
      )}
